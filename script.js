...
const formatedMovementDate = (date, locale) => {
  ...
  const daysPassed = calcDaysPassed(new Date(2020, 6, 14), date);
...
const displayMovements = function (acc, sort = false) {
  ...
  combinedMovsDate.forEach(function ({ movement, movementDate }, i) {
    ...
    const displayDay = formatedMovementDate(date, acc.locale);

    // const formatedMovement = new Intl.NumberFormat(acc.locale, {
    //   style: 'currency',
    //   currency: acc.currency,
    // }).format(movement);

    const formatedMovement = formatedCur(movement, acc.locale, acc.currency);
...
const calcDisplayBalance = function (acc) {
  ...
  const formattedBalance = formatedCur(acc.balance, acc.locale, acc.currency);
...
// Functio return a number between a minimum and maximum specified value, with both the minimum and maximum values inclusive
...
console.log(new Date(2025, 0, 7, 23, 45, 5));
...
console.log(calcDaysPassed(new Date(2023, 4, 11), new Date(2023, 4, 1)));

// number internalization

const num = 34444447.67;
console.log(new Intl.NumberFormat(navigator.language).format(num));

// Timer

// setTimeout(
//   (sub, title) => console.log(` time up ⌛submit your ${sub} ${title}`),
//   3000,
//   'Chemistry',
//   'practical'
// );
